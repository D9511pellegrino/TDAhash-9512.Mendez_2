...
void *lista_buscar_elemento(lista_t *lista, int (*comparador)(void *, void *),
				void *contexto)
{
	if(!lista || !comparador) return 0;
	size_t cantidad_llamadas_realizadas = 0;

	nodo_t* nodo_actual = lista->nodo_inicio;

	while (cantidad_llamadas_realizadas < lista->cantidad){
		if(comparador(nodo_actual->elemento, contexto)==0){
			return nodo_actual->elemento;
		} else{
			nodo_actual = nodo_actual->siguiente;
			cantidad_llamadas_realizadas++;
		}   
	}
	return NULL;
}
...
void lista_destruir_todo(lista_t *lista, void (*funcion)(void *))
{
	...
	for(size_t i=0; i<lista->cantidad; i++){
		aux = nodo_liberar->siguiente;
		if(funcion) funcion(nodo_liberar->elemento);
		free(nodo_liberar);
		nodo_liberar = aux;
	}
...
size_t lista_con_cada_elemento(lista_t* lista, bool (*funcion)(void*, void*), void *contexto)
{
	...
	while (llamar_funcion && (cantidad_llamadas_realizadas < lista->cantidad)){
		llamar_funcion = funcion(nodo_actual->elemento, contexto);
		nodo_actual = nodo_actual->siguiente;
		cantidad_llamadas_realizadas++;    
	}

	return cantidad_llamadas_realizadas;
}
